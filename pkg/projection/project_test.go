...
func TestTransform(t *testing.T) {
		...
		"transform Points object", func(t *testing.T) {
			...
			t.Run(
				"project to lambert", func(t *testing.T) {
					...
					new_p, err := Transform(&p, lambert_wkt, false, nil) // Transform to Lambert
			...
			t.Run(
				"project back lambert to 4326", func(t *testing.T) {
					...
					test_p, err := Transform(new_p, "EPSG:4326", true, nil) // Transform back to 4326
	...
	t.Run(
		"project LRSRoute object", func(t *testing.T) {
			...
			new_lrs, err := Transform(&lrs, "EPSG:4326", true, nil)
			if err != nil {
				t.Error(err)
			}
			new_lrs.Release()
	...
	t.Run(
		"Project LRS Route event", func(t *testing.T) {
			...
			geom_out, err := Transform(event, geom.LAMBERT_WKT, true, nil)
			if err != nil {
				t.Error(err)
			}
